...
if (isset($_POST['delete_id'])) {
    $id = $_POST['delete_id'];
    $delete_stid = oci_parse($conn, "DELETE FROM Flotte WHERE ID_FLOTTE = :id");
    oci_bind_by_name($delete_stid, ":id", $id);
    oci_execute($delete_stid);
    oci_free_statement($delete_stid);
    oci_close($conn);
    header("location:info_flotte.php");
    exit();
}
...
<body>
    <div class="table-container">
        <h2 class="text-center mb-4">Liste des Flottes</h2>
            ...
            <tbody>
                ...
                <?php while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) : ?>
                    <tr id="row_<?php echo $row['ID']; ?>">
                        ...
                        <td>
                        <form method="post">
        			<input type="hidden" name="delete_id" value="<?php echo $row['ID_FLOTTE']; ?>">
