...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    $stid = oci_parse($conn, "SELECT * FROM entrepot  WHERE ID_ENTREPOT = :id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['modifier'])) {
    $id = $_POST['id'];
    $new_id = isset($_POST['ID_ENTREPOT']) && !empty($_POST['ID_ENTREPOT']) ? $_POST['ID_ENTREPOT'] : $id;
    $NOM= isset($_POST['NOM']) && !empty($_POST['NOM']) ?$_POST['NOM'] : $req_actuel['NOM'];
    $REGION = isset($_POST['REGION']) && !empty($_POST['REGION']) ? $_POST['REGION'] : $req_actuel['REGION'];

    $update_stid = oci_parse($conn, "UPDATE entrepot SET ID_ENTREPOT = :new_id,NOM=:NOM ,REGION = :REGION WHERE ID_ENTREPOT = :id");
    oci_bind_by_name($update_stid, ":new_id", $new_id);
    oci_bind_by_name($update_stid, ":NOM", $NOM);
    oci_bind_by_name($update_stid, ":REGION", $REGION);
...
<head>
    ...
    <title>Modifier un entrepot 🎅</title>
...
<div class="container">
    <h2 class="text-center">🎄 Modifier un entrepot 🎄</h2>
    <?php echo $message; ?>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <div class="mb-3">
            <label class="form-label">ID entrepot (Obligatoire) 🎅</label>
    ...
    <?php if ($req_actuel): ?>
        ...
        <form method="POST" class="mt-4">
            <input type="hidden" name="id" value="<?php echo htmlentities($req_actuel['ID_ENTREPOT']); ?>">

            <div class="mb-3">
                <label class="form-label">ID Actuel : <?php echo htmlentities($req_actuel['ID_ENTREPOT']); ?> 🎅</label>
            ...
            <div class="mb-3 hidden" id="champs-new-id">
                ...
                <input type="text" class="form-control" name="ID_ENTREPOT">
            </div>
            ...
            <div class="mb-3">
                <label class="form-label">🎄 REGION (Actuel : <?php echo htmlentities($req_actuel['REGION']); ?>)</label>
                <input type="text" class="form-control" name="REGION" placeholder="Laisser vide pour conserver l'ancien">
            </div>
