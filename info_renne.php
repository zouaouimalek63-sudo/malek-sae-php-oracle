...
if (isset($_POST['delete_id'])) {
    $id = $_POST['delete_id'];
    $delete_stid = oci_parse($conn, "DELETE FROM renne WHERE PUCE =:id");
    oci_bind_by_name($delete_stid, ":id", $id);
    oci_execute($delete_stid);
    oci_free_statement($delete_stid);
    oci_close($conn);
    header("location:info_renne.php");
    exit();
}

$stid = oci_parse($conn, 'SELECT * FROM RENNE');
oci_execute($stid);
?>
...
<body>
    <div class="table-container">
        <h2 class="text-center mb-4">Liste des Rennes</h2>
            ...
            <tbody>
                <?php while ($row = oci_fetch_array($stid, OCI_ASSOC+OCI_RETURN_NULLS)) : ?>
                    <tr id="row_<?php echo $row['PUCE']; ?>">
                        ...
                        <td>
                            <form method="post">
                                <input type="hidden" name="delete_id" value="<?php echo $row['PUCE']; ?>">
