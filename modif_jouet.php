...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    $stid = oci_parse($conn, "SELECT * FROM jouet  WHERE ID_JOUET = :id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['modifier'])) {
    $id = $_POST['id'];
    $new_id = isset($_POST['ID_JOUET']) && !empty($_POST['ID_JOUET']) ? $_POST['ID_JOUET'] : $id;
    $TYPE_J= isset($_POST['TYPE_J']) && !empty($_POST['TYPE_J']) ?$_POST['TYPE_J'] : $req_actuel['TYPE_J'];

    $update_stid = oci_parse($conn, "UPDATE jouet SET ID_JOUET = :new_id,TYPE_J=:TYPE_J WHERE ID_JOUET = :id");
    oci_bind_by_name($update_stid, ":new_id", $new_id);
    oci_bind_by_name($update_stid, ":TYPE_J", $TYPE_J);    
...
<div class="container">
    <h2 class="text-center">🎄 Modifier un jouet 🎄</h2>
    <?php echo $message; ?>

    <!-- Formulaire de recherche -->
    <form method="POST">
        <div class="mb-3">
            <label class="form-label">ID jouet (Obligatoire) 🎅</label>
    ...
    <?php if ($req_actuel): ?>
        ...
        <form method="POST" class="mt-4">
            <input type="hidden" name="id" value="<?php echo htmlentities($req_actuel['ID_JOUET']); ?>">

            <div class="mb-3">
                <label class="form-label">ID Actuel : <?php echo htmlentities($req_actuel['ID_JOUET']); ?> 🎅</label>
            ...
            <div class="mb-3 hidden" id="champs-new-id">
                ...
                <input type="text" class="form-control" name="ID_JOUET">
            </div>

            <div class="mb-3">
                <label class="form-label">🎅 TYPE_J (Actuel : <?php echo htmlentities($req_actuel['TYPE_J']); ?>)</label>
                <input type="text" class="form-control" name="TYPE_J" placeholder="Laisser vide pour conserver l'ancien" >
            </div>
